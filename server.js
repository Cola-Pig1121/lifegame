...
app.post('/api/test-connection', async (req, res) => {
  try {
    ...
    if (!baseUrl || !apiKey || !model) {
      return res.status(400).json({ error: '请先填写Base URL、选择模型和API Key' });
    }

    console.log('测试连接请求:', { baseUrl, model });
    ...
    while (retries <= maxRetries) {
      try {
        console.log(`开始API测试调用... (尝试 ${retries + 1}/${maxRetries + 1})`);
        const response = await client.chat.completions.create({
          model: model,
          messages: [{ role: "user", content: "你好，这是一个连接测试。" }],
          max_tokens: 5, // 减少token数量，降低API负载
          temperature: 0.7,
          stream: false
        });
        
        console.log('API测试调用成功:', response.choices[0]?.message?.content);
        return res.json({ 
          success: true, 
          message: "连接测试成功！", 
          model: model,
